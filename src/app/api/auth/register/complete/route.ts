...
import { completeRegistration, rateLimit, signSession } from "@/src/lib/authStore";
...
export async function POST(request: Request) {
  ...
  const limited = rateLimit(`register-complete:${getClientKey(request)}:${body?.phone ?? ""}`, 12, 15 * 60 * 1000);
  if (!limited.ok) {
  ...
  if (!registrationToken) {
    return NextResponse.json({ ok: false, error: "کد تایید هنوز کامل نشده است." }, { status: 400 });
  }

  if (password.length < 8) {
    return NextResponse.json({ ok: false, error: "رمز عبور باید حداقل ۸ کاراکتر باشد." }, { status: 400 });
  }

  const result = await completeRegistration(phone.value, registrationToken, displayName, password);
  if (!result.ok) {
