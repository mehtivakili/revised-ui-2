...
function categoryById(id: DashboardCategory["id"]) {
    ...
    <Link className={`tool-card ${locked ? "tool-card-locked" : ""}`} href={locked ? "/profile?upgrade=1" : `/calculators/${tool.slug}`}>
      <div className="tool-card-head">
        ...
        <div className="tool-icon-wrapper">
          ...
          {locked ? <Lock className="tool-icon-lock-overlay" size={16} aria-hidden="true" /> : null}
        </div>
      </div>
      ...
      <div className="tool-card-foot">
        <span>{locked ? "ارتقای حساب" : "شروع محاسبه"}</span>
        <span className="tool-card-arrow">
          {locked ? <Lock size={14} aria-hidden="true" /> : <ChevronLeft size={15} aria-hidden="true" />}
        </span>
      </div>
    </Link>
  );
